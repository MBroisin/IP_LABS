...
public class Code {
	...
	public ImageAccess synthesize(ImageAccess in, int n, int order) {
		...
		ImageAccess LL = upsample(convolve(in.getSubImage(0, 0, in.nx/2, in.ny/2), Hainv, Hainv));
		ImageAccess HL = upsample(convolve(in.getSubImage(in.nx/2, 0, in.nx/2, in.ny/2), Hainv, Gainv));
		ImageAccess LH = upsample(convolve(in.getSubImage(0, in.ny/2, in.nx/2, in.ny/2), Gainv, Hainv));
		ImageAccess HH = upsample(convolve(in.getSubImage(in.nx/2, in.ny/2, in.nx/2, in.ny/2), Gainv, Gainv));
