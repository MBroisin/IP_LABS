...
public class Detect_Ridges implements PlugIn {
	...
	public void run(String arg) {
		...
		GenericDialog gd = new GenericDialog("Detect Ridges");
		gd.addNumericField("Sigma", 3, 1);
		gd.addNumericField("Threshold Low", 5.5, 1);
		gd.addNumericField("Threshold High", 10.5, 1);
		...
		ImageAccess out = (new Code()).ridges(input, sigma, tL, tH);
		if (gd.getNextBoolean())
			OverlayDetection.show(imp, out);
		else 
			out.show("Detect Ridges (" + sigma + ")");
